...
func _init() -> void:
	...
	_grid_card_spacing = Vector2(0.8, 1)
	...
	_fine_angle_min = deg2rad(-5)
	_fine_angle_max = deg2rad(5)

	# Scale fine tuning
	_fine_scale = false
	_fine_scale_mode = FineTuningMode.SYMMETRIC
	_fine_scale_ratio = AspectMode.KEEP
	_fine_scale_min = Vector2(1, 1)
	_fine_scale_max = Vector2(0.8, 0.8)
