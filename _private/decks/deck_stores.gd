class_name DeckStores
extends Node

const DECK_STORE_PATH: String = "res://_private/decks/decks.data"
...
func _get_stores() -> Dictionary:
	...
	var err = file.load(DECK_STORE_PATH)
	if err != OK:
...
func _post_store(id: String, name: String, cards: Array) -> void:
	var file = ConfigFile.new()
	file.load(DECK_STORE_PATH)

	file.set_value(id, "name", name)
	file.set_value(id, "cards", cards)
	var err = file.save(DECK_STORE_PATH)
	if err != OK:
