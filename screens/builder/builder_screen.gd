...
onready var _deck_select = $BuilderLayout/DeckBg/CardDrop/DeckLayout/StoreOrDeck/DeckSelect
...
func _ready() -> void:
	...
	_store_select.clear()
	_store_select.add_item("Sys")
	_store_select.add_item("Usr")
	_update_deck_select()
...
func _update_deck_select() -> void:
	_deck_select.clear()
	if _store_mode == "sys":
		var decks = SysDeckStores.saved_stores()
...
func _on_DeckSelect_item_selected(index: int) -> void:
	...
	_deck.clear()
	if _store_mode == "sys":
		SysDeckStores.load_store(_deck_select.get_selected_metadata(), _deck)
	elif _store_mode == "usr":
...
func _on_SaveBtn_pressed() -> void:
	...
	if _store_mode == "sys":
		SysDeckStores.save_store(id, _deck_name.text, _deck)
	elif _store_mode == "usr":
...
func _on_StoreSelect_item_selected(index: int) -> void:
	if index == 0:
		_store_mode = "sys"
	elif index == 1:
		_store_mode = "usr"
